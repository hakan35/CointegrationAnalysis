...
# Inclusion of the dummy*trend interaction term is exactly the same as the "const" or "trend" implementation
# Additional possibility to define structural breaks, so that they are considered in this.

# break.matrix is a matrix containing the possible break in the cointegrating relationship.
# H_c(r) - Johansen et al. (2000) Shift in intercept model, none of the p time series have a deterministic trend
...
ca.jomoni <- function (x, type = c("eigen", "trace"), ecdet = c("nc", "rc", "uc", "crt", "ct"), K = 2, spec = c("longrun", "transitory"), season = NULL, dumvar = NULL,  break.matrix = NULL)
{
  ...
  if (!(is.null(break.matrix))){
    warning("Inclusion of breaks in the cointegration vector requires different critical values!")
  }
  x <- na.omit(x)
  N <- nrow(x)
  Z <- embed(diff(x), K)
