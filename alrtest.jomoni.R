alrtest.jomoni <- function (z, A, r){
  ...
  type <- "Estimation and testing under linear restrictions on alpha"
  ...
  valeigen <- eigen(Cinv %*% Ska.b %*% Saa.binv %*% Sak.b %*% 
                      t(Cinv), symmetric = TRUE)
  ...
  new("cajo.test", Z0 = z@Z0, Z1 = z@Z1, ZK = z@ZK, ecdet = z@ecdet, 
      H = NULL, A = A, B = B, type = type, teststat = teststat, 
      pval = pval, lambda = lambda.res, Vorg = Vorg, V = V, 
      W = ALPHA, PI = PI, DELTA = NULL, DELTA.bb = DELTA.bb, 
      DELTA.ab = DELTA.ab, DELTA.aa.b = DELTA.aa.b, GAMMA = GAMMA, 
      test.name = "Johansen-Procedure")
}

A <- matrix(c(1,0,0
), nrow = 3)
alrtest.jomoni(z, A = A, r = 1)
summary(alrtest.jomoni(z, A = A, r = 1))